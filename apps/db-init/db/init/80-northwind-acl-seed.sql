...
-- Products
INSERT INTO acl.objects(resource_type, resource_pk)
SELECT 'product', product_id::text FROM products
ON CONFLICT DO NOTHING;
...
INSERT INTO acl.objects(resource_type, resource_pk)
SELECT 'order_detail', (order_id::text || ':' || product_id::text) FROM order_details
ON CONFLICT DO NOTHING;
...
INSERT INTO acl.tuples(principal_id, relation, resource_type, resource_pk)
SELECT g.id, 'manager', 'customer', c.customer_id     FROM g, customers c
UNION ALL
SELECT g.id, 'manager', 'order',    o.order_id::text  FROM g, orders o
UNION ALL
SELECT g.id, 'manager', 'product',  p.product_id::text FROM g, products p
ON CONFLICT DO NOTHING;
...
JOIN products pr ON cp.kind = 'customer'
ON CONFLICT DO NOTHING;
...
JOIN products pr ON ep.kind = 'employee'
UNION ALL
SELECT ep.id, 'viewer', 'customer', c.customer_id
FROM acl.principals ep
JOIN customers c ON ep.kind = 'employee'
ON CONFLICT DO NOTHING;
