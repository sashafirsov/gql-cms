...
describe('Northwind Authentication API', () => {
  const API_BASE = '/northwind/auth';
  ...
  describe('POST /northwind/auth/register', () => {
    it('should successfully register a new customer', async () => {
      ...
      expect(res.data.principal).toMatchObject({
        email,
        kind: 'customer',
        displayName: 'Test Customer',
        emailVerified: false,
      });
      expect(res.data.principal.id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
    ...
    it('should register an employee with appropriate role', async () => {
      const email = generateEmail();
      const password = 'SecurePass123!';
      ...
      expect(res.status).toBe(201);
      expect(res.data.success).toBe(true);
      expect(res.data.principal.kind).toBe('employee');
    });

    it('should use email as displayName when not provided', async () => {
      const email = generateEmail();
      const password = 'SecurePass123!';
      ...
      expect(res.status).toBe(201);
      expect(res.data.principal.displayName).toBe(email);
    });
    ...
    it('should fail with duplicate email', async () => {
      ...
      try {
        ...
        expect(errorMessage.toLowerCase()).toMatch(/email|exists|duplicate|conflict/);
      }
    });
  ...
  describe('POST /northwind/auth/login', () => {
  ...
  describe('GET /northwind/auth/me', () => {
    let testEmail: string;
    let testPassword: string;
  ...
  describe('POST /northwind/auth/logout', () => {
    let accessToken: string;
  ...
  describe('POST /northwind/auth/refresh', () => {
    let refreshToken: string;
    ...
    beforeAll(async () => {
      ...
      const cookies = extractCookies(regRes.headers['set-cookie'] || []);
      refreshToken = cookies.get('refresh_token') || '';
      principalId = regRes.data.principal.id;
    });
  ...
  describe('Integration: Complete auth flow', () => {
    it('should complete full registration -> login -> access protected resource -> logout flow', async () => {
      ...
      const principalId = regRes.data.principal.id;
      ...
      expect(meRes.data.principal.id).toBe(principalId);
      expect(meRes.data.principal.email).toBe(email);
  ...
  describe('Security: Cookie attributes', () => {
    ...
    it('should set appropriate cookie paths', async () => {
      ...
      const refreshCookie = setCookieHeaders.find(c => c.startsWith('refresh_token='));
      expect(refreshCookie).toContain('Path=/northwind/auth');
    });
  });
});
